...
export default function FuelRecap() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!isAdminOrSuper && user?.area_id && filters.area_id.length === 0) {
  ...
  const handleResetFilters = () => {
    const initial = {
      area_id: isAdminOrSuper ? [] : user?.area_id ? [String(user.area_id)] : [],
      sto_id: "",
      start_date: "",
      end_date: "",
    };
  ...
  const totals = useMemo(() => {
    ...
    return {
      distance: selected.reduce(
        (acc, curr) => acc + parseFloat(curr.distance || 0),
        0,
      ),
      cost: selected.reduce(
        (acc, curr) => acc + parseFloat(curr.fuel_cost || 0),
        0,
      ),
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th
                      className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-blue-600"
                    ...
                    <th className="px-6 py-4 w-12 text-center">
                      <button
                        onClick={handleSelectAll}
                        className="text-blue-600 transition-transform active:scale-90"
                        title="Pilih Semua"
                      >
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          {editingId === r.id ? (
                            ...
                            <div className="flex items-center justify-center gap-2">
                              <span
                                className="text-xs font-black text-emerald-600 cursor-pointer"
                                onClick={() => handleEdit(r)}
                                title="Klik untuk edit"
                              >
                                Rp {r.fuel_cost?.toLocaleString()}
                              </span>
                              {r.fuel_receipt && (
                                <button
                                  onClick={() =>
                                    setShowImageModal(r.fuel_receipt)
                                  }
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            ...
                            className={`${selectedIds.includes(r.id) ? "text-blue-600" : "text-slate-300"}`}
                          >
            ...
            <div className="md:hidden divide-y divide-slate-100">
              {reports.length > 0 && !loading && (
                <div className="bg-slate-50/50 px-4 py-3 flex items-center justify-between">
                  ...
                  <button
                    onClick={handleSelectAll}
                    className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white border border-slate-200 shadow-sm text-blue-600 transition-all active:scale-95"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-slate-50 rounded-2xl p-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {r.fuel_receipt && (
                            <button
                              onClick={() => setShowImageModal(r.fuel_receipt)}
